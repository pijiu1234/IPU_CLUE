...
class AdamWeightDecayOptimizer(tf.train.Optimizer):
    ...
    def __init__(self,
        ...
        self.beta_2 = tf.cast(beta_2, tf.float16)
    ...
    def apply_gradients(self, grads_and_vars, global_step=None, name=None):
        ...
        for (grad, param) in grads_and_vars:
            ...
            grad_fp32 = tf.cast(grad, tf.float16)
...
class LAMBOptimizer(tf.train.Optimizer):
    ...
    def forward_transform(self, input_tensor):
        ...
        # [H, 3H] -> [H, 3, H] -> [3, H^2]
        # TODO: This is just for `qkv_weight`, not for `qkv_bias`. Add transformer for bias.
...
def get_optimizer(learning_rate, FLAGS):
    loss_scaling = FLAGS.loss_scaling
    if FLAGS.reduction_type == "mean":
        loss_scaling *= FLAGS.pipeline_depth
    scaled_learning_rate = learning_rate / FLAGS.loss_scaling
    if FLAGS.optimizer_type.lower() == 'sgd':
        optimizer = tf.train.GradientDescentOptimizer(scaled_learning_rate)
    elif FLAGS.optimizer_type.lower() == 'momentum':
        optimizer = tf.train.MomentumOptimizer(scaled_learning_rate,
                                               momentum=FLAGS.momentum,
                                               use_nesterov=False)
    elif FLAGS.optimizer_type.lower() == 'adam':
        optimizer = tf.train.AdamOptimizer(scaled_learning_rate,
                                           beta1=FLAGS.beta1,
                                           beta2=FLAGS.beta2,
                                           epsilon=FLAGS.epsilon)
    elif FLAGS.optimizer_type.lower() == 'adamw':
        optimizer = AdamWeightDecayOptimizer(learning_rate,
                                             loss_scaling=loss_scaling,
                                             beta_1=FLAGS.beta1,
                                             beta_2=FLAGS.beta2,
                                             epsilon=FLAGS.epsilon)
    elif FLAGS.optimizer_type.lower() == 'lamb':
    ...
    elif FLAGS.optimizer_type.lower() == 'custom':
        ...
        optimizer = StageMomentumOptimizer(learning_rate, FLAGS.momentum,
                                           tvars, stage_weights=stage_weights)
    else:
        raise ValueError(f"Optimizer {FLAGS.optimizer} not recognised")
    return optimizer
