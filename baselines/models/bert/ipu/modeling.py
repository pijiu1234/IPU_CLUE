...
class BertConfig(object):
    ...
    def __init__(self,
        ...
        self.intermediate_size = hidden_act*4
...
class BertModel(object):
    ...
    def embedding_lookup_layer(self,
        ...
        with tf.variable_scope("bert"):
            with tf.variable_scope("embeddings"):
                word_embeddings = self.embedding(
                    input_ids, self.bert_config.vocab_size, name="word_embeddings", num_splits=4)
    ...
    def get_classifier_output_layer(self,input_tensor,label_ids,is_real_example):
        with tf.variable_scope('bert'):
            pooled_output = self.pooler(input_tensor)
        hidden_size = pooled_output.shape[-1].value

        output_weights = tf.get_variable(
            "output_weights", [self.bert_config.label_num, hidden_size],
            initializer=tf.truncated_normal_initializer(stddev=0.02),dtype=self.bert_config.dtype)

        output_bias = tf.get_variable(
            "output_bias", [self.bert_config.label_num], initializer=tf.zeros_initializer(),dtype=self.bert_config.dtype)
        ...
        with tf.variable_scope("loss"):
            ...
            one_hot_labels = tf.one_hot(label_ids, depth=self.bert_config.label_num, dtype=self.bert_config.dtype)
        ...
        if self.is_training:
            ...
            return {"loss":per_example_loss,"label_ids":label_ids,"logits":logits,"is_real_example":is_real_example}
